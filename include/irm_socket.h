...
enum {
    IRM_SOCKET_TYPE_EFVI = 0,
    IRM_SOCKET_TYPE_NATIVE
};
...
typedef void (*log_handle) (int log_level, const char *msg,
     size_t msg_size);
...
IRM_SOCK_HOT_CALL void *
irm_pub_alloc(IRM_PUBHANDLE handle, size_t * const max_size);
IRM_SOCK_HOT_CALL int
irm_pub_send(IRM_PUBHANDLE handle, void *data, size_t data_len);
IRM_SOCK_HOT_CALL int
irm_pub_free(IRM_PUBHANDLE handle, void *data);
...
IRM_SOCK_HOT_CALL void * const
irm_sub_recv(IRM_SUBHANDLE handle, size_t * const data_len);
IRM_SOCK_HOT_CALL int
irm_sub_free(IRM_SUBHANDLE handle, void *data);
uint8_t irm_sub_getalivedpubs(IRM_SUBHANDLE handle);

void irm_set_log(log_handle callback, int log_level);
