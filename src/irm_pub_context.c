...
#if 0
static void irm_pub_context_heartbeat_init(struct irm_pub_context *ctx);
static void irm_pub_context_heartbeat(struct irm_pub_context *ctx);
#endif
...
#if 0
    irm_pub_context_heartbeat_init(ctx);
#endif
    ...
    struct irm_mbuf *mbuf)
{
    ...
    IRM_DBG("put mbuf id %u, tail %u, head %u, count %u", mbuf->id,
        ctx->netio->rx_pool.tail,
        ctx->netio->rx_pool.head,
        ctx->netio->rx_pool.count);
    ...
    const uint64_t heartbeat_timeout)
{
    ...
    /* 
    IRM_DBG("last_send_seq %ld, breakpoint_seq %u", last_send_seq,
        ctx->breakpoint_seq); 
    */ 

    mbuf = ctx->reserved_mbufs[IRM_PUB_BREAKPOINT_MBUF_ID];
    if (mbuf->status != IRM_MBUF_STATUS_IDLE) {
        //IRM_DBG("breakpoint mbuf is sending, id %u", mbuf->id);
        return;
    }

    if (last_send_seq < 0) {
        //IRM_DBG("last_send_seq is 0, maybe not send yet");
        heartbeat = IRM_TRUE;
        last_send_seq = 0;
    }

    if (irm_txbuffer_available(ctx->netio->tx_buffer)) {
        //IRM_DBG("breakpoint tx buffer not empty");
        heartbeat = IRM_TRUE;
        last_send_seq = 0;
    }
...
irm_pub_context_msg_handle(void *ctx, struct irm_mbuf *mbuf)
{
    ...
    if (IRM_UNLIKELY(header->role == IRM_ROLE_TYPE_PUB)) {
        ...
        IRM_DBG("mbuf id %u, tail %u, head %u, count %u", mbuf->id,
            pub_ctx->netio->rx_pool.tail,
            pub_ctx->netio->rx_pool.head,
            pub_ctx->netio->rx_pool.count);
        return IRM_OK;
    }
...
IRM_HOT_CALL static void * irm_pub_context_event_loop(void *arg)
{
    ...
    while (!ctx->quit) {
        ...
        */
        ...
        /*
        IRM_DBG("curr_ts %lu, idle_ts %lu, delta_ts %lu",
            curr_ts, idle_ts, delta_ts);
        */ 
