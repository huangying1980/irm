...
#if 0
IRM_HOT_CALL static IRM_ALWAYS_INLINE void irm_sub_context_heartbeat_handle(
    struct irm_sub_context *ctx, struct irm_mbuf *mbuf);
#endif
...
#if 0
IRM_HOT_CALL static IRM_ALWAYS_INLINE void irm_sub_context_nack_check(
    struct irm_sub_context *ctx, uint64_t curr_ts);
#endif
...
#if 0
IRM_HOT_CALL static void irm_sub_context_cache_clear(
    struct irm_sub_context *ctx);
#endif
...
static int irm_sub_config_check(const struct irm_config *cfg)
{
    ...
    if (!cfg->addr.mcgroup_ip || !cfg->addr.mcgroup_ip[0]) {
    ...
    if (!cfg->addr.local_ip || !cfg->addr.local_ip[0]) {
        IRM_ERR("local ip config error");
        return -IRM_ERR_CONFIG;
    }
    if (cfg->io_mode != IRM_SOCKET_TYPE_EFVI
...
#if 0
IRM_HOT_CALL static IRM_ALWAYS_INLINE void
irm_sub_context_heartbeat_handle(struct irm_sub_context *ctx,
    struct irm_mbuf *mbuf)
{
#if defined IRM_DEBUG || IRM_DEBUG_VERBOSE
    struct irm_msg_header *header;
    header = IRM_MBUF_MSG(irm_msg_header, mbuf, ctx->netops.payload_offset);
    ...
    struct irm_mbuf *mbuf)
{
    ...
    /*
    if (desc->nack.count) {
        IRM_DBG("breakpoint already nack sender_id %u, "
            "start %u, end %u, count %u", sender_id,
            desc->nack.start, desc->nack.end, desc->nack.count);
        return;
    }
    */
...
#ifdef  IRM_TRACE
            if (mbuf == iter_mbuf || mbuf->id == iter_mbuf->id) {
                IRM_DBG("sender_id %u, header seq %u iter_header seq %u repeat drop same, lock %d, "
                    "ref %d, mbuf %p, mbuf id %u, mbuf self %p, mbuf->prev %p, mbuf->next %p, "
                    "iter_mbuf %p, iter_mbuf id %u, iter self %p, iter->prev %p, iter->next %p",
                    header->sender_id, header->seq, iter_header->seq, mbuf->lock, mbuf->ref, mbuf,
                    mbuf->id, &mbuf->sender_ln, mbuf->sender_ln.prev, mbuf->sender_ln.next,
                    iter_mbuf, iter_mbuf->id, &iter_mbuf->sender_ln, iter_mbuf->sender_ln.prev,
                    iter_mbuf->sender_ln.next);
            }
#endif
...
#if 0
IRM_HOT_CALL static IRM_ALWAYS_INLINE void
irm_sub_context_nack_check(struct irm_sub_context *ctx, uint64_t curr_ts)
{
    struct irm_pub_desc *desc = ctx->pubs->desc;
    uint8_t              count = ctx->pubs->alive_count;
    uint8_t             *alives = ctx->pubs->alives;
    uint8_t              i;
    uint8_t              src_id;
    ...
    for (i = 0; i < count; ++i) {
        src_id = alives[i];
        if (!IRM_QUEUE_EMPTY(&desc[src_id].cache.cache_list)) {
            IRM_DBG("src_id %u desc not cache_list empty", src_id);
            continue;
        }
        if (!desc[src_id].nack.count) {
            IRM_DBG("src_id %u, no nack in sending", src_id);
            continue;
        }

        if (curr_ts - desc[src_id].nack.ts <= ctx->renack_timeout) {
            IRM_DBG("curr_ts %lu, nack.ts %lu, renacke_timeout %lu, src_id %u",
                curr_ts, desc[src_id].nack.ts, ctx->renack_timeout, src_id);
            continue;
        }

        IRM_DBG("curr_ts %lu, nack.ts %lu, renacke_timeout %lu, src_id %u, " 
            "send nack", curr_ts, desc[src_id].nack.ts, ctx->renack_timeout,
            src_id);

        irm_sub_context_nack(ctx, src_id, curr_ts);
    }
}
#endif
...
#if 0
    if (msg->header.target_id != ctx->self_id) {
        IRM_DBG("lost data target_id %u not match, self_id %u, seq %u, drop",
            msg->header.seq, msg->header.target_id, ctx->self_id);
        irm_mbuf_put(&ctx->netio->rx_pool, mbuf);
        return;
    }
#endif
...
irm_sub_context_msg_handle(void *ctx, struct irm_mbuf *mbuf)
{
    ...
    switch (header->msg_type) {
        ...
        case IRM_MSG_TYPE_HEARTBEAT:
            irm_sub_context_heartbeat_handle(sub_ctx, mbuf);
            break;
#endif
...
#if 0
IRM_HOT_CALL static void
irm_sub_context_cache_clear(struct irm_sub_context *ctx)
{
    struct irm_pub_desc   *desc;
    ...
    struct irm_mbuf       *dmbuf;
    struct irm_queue      *diter;
    struct irm_msg_header *dheader;
    ...
    IRM_QUEUE_FOREACH(iter, &ctx->cache_list) {
        ...
        header = IRM_MBUF_MSG(irm_msg_header, mbuf, offset);
        desc = &ctx->pubs->desc[header->sender_id];
        if (desc->online && desc->token == header->token) {
            IRM_DBG("token matched sender_id %u", header->sender_id);
            break;
        }
        IRM_DBG("remove offline or token %u %u dismatch, sender_id %u, seq %u, "
            "self %p, prev %p, next %p", desc->token, header->token,
            header->sender_id, header->seq, iter, iter->prev, iter->next);

        IRM_SUB_DEL_CACHE(ctx, desc, iter, &mbuf->sender_ln);
        irm_mbuf_put(&ctx->netio->rx_pool, mbuf);

        if (IRM_QUEUE_EMPTY(&desc->cache.cache_list)) {
            IRM_DBG("offline id %u desc cache_list empty", header->sender_id);
            desc->cache.min = 0;
            desc->cache.max = 0;
            continue;
        }
        diter = IRM_QUEUE_HEAD(&desc->cache.cache_list);
        dmbuf = IRM_QUEUE_DATA(diter, struct irm_mbuf, sender_ln);
        dheader = IRM_MBUF_MSG(irm_msg_header, dmbuf, offset); 
        IRM_DBG("offline id %u, update cache.min %u to seq %u",
            dheader->sender_id, desc->cache.min, dheader->seq);
        desc->cache.min = dheader->seq;
    }
}
#endif
IRM_HOT_CALL static IRM_ALWAYS_INLINE void
...
IRM_HOT_CALL static void *irm_sub_context_event_loop(void *arg)
{
    ...
    while (!ctx->quit) {
        ...
        /*
        IRM_DBG("curr_ts %lu idle_ts %lu, idle_timeout %lu alive_timeout %lu, "
            "heartbeat_timeout %lu", curr_ts, idle_ts, idle_timeout,
            alive_timeout, heartbeat_timeout);
        */
#if 0
        if (!IRM_SUB_ALIVE_PUB_COUNT(ctx)) {
